# 碳排放數據種子資料

這個資料夾包含自動生成的碳排放數據，用於測試和開發。

## 📋 文件說明

### carbon-emissions-seed.json
自動生成的過去 6 個月（180天）的碳排放數據，包含：
- **數據點**: 180 筆（每天一筆）
- **時間範圍**: 過去 6 個月
- **數據內容**:
  - Scope 1, 2, 3 碳排放分類
  - 電力、天然氣、燃料、運輸、廢棄物、用水等細分數據
  - 自動生成的週末效應（週末排放較低）
  - 季節性變化（夏季和冬季能源使用較高）

## 🚀 使用方法

### 方法 1: 直接導入數據庫（推薦）

如果您已經配置好資料庫，可以直接使用導入腳本：

```bash
# 1. 確保已設置 DATABASE_URL 環境變數
export DATABASE_URL="postgresql://user:password@localhost:5432/carbon_db"

# 2. 生成 Prisma Client
npx prisma generate

# 3. 推送資料庫結構
npx prisma db push

# 4. 導入數據
npx ts-node scripts/import-seed-data.ts
```

### 方法 2: 使用 Prisma Seed 腳本

```bash
# 1. 確保已設置 DATABASE_URL 環境變數
export DATABASE_URL="postgresql://user:password@localhost:5432/carbon_db"

# 2. 執行 seed 腳本（會生成 180 天的數據）
npm run prisma:seed
```

### 方法 3: 查看 JSON 數據

您可以直接查看 `carbon-emissions-seed.json` 文件來了解數據結構：

```bash
# 查看數據摘要
cat data/carbon-emissions-seed.json | jq '.metadata'

# 查看前 5 筆數據
cat data/carbon-emissions-seed.json | jq '.data[:5]'
```

## 🎨 前端日期篩選功能

前端儀表板已經支持日期篩選功能：

### API 端點

```
GET /api/carbon/dashboard?startDate=2025-05-10&endDate=2025-11-05
```

**查詢參數**:
- `startDate`: 開始日期（格式: YYYY-MM-DD）
- `endDate`: 結束日期（格式: YYYY-MM-DD）
- 如果不提供，預設顯示過去 30 天的數據

### 前端功能

儀表板頁面（`/dashboard`）提供以下功能：

1. **快速日期選擇器**: 可快速選擇過去 7 天、30 天、90 天、180 天或今年的數據
2. **自定義日期範圍**: 可手動選擇開始和結束日期
3. **實時數據更新**: 每 5 秒自動刷新數據
4. **視覺化圖表**:
   - 趨勢圖：顯示時間序列的碳排放變化
   - 圓餅圖：顯示 Scope 1/2/3 的比例
   - 柱狀圖：比較不同類別的排放量

## 📊 數據統計

生成的數據包含以下統計資訊：

- **總碳排放**: ~69,539 tCO2e（過去 180 天）
- **平均每日**: ~386 tCO2e
- **最高單日**: ~487 tCO2e
- **最低單日**: ~250 tCO2e

數據特性：
- 包含週末效應（週末排放降低約 30%）
- 包含季節性變化（夏季和冬季增加約 15%）
- 每日數據在基準值上有隨機波動

## 🔧 自定義數據生成

如果您需要生成不同時間範圍的數據：

```bash
# 編輯 scripts/generate-seed-data.ts
# 修改 generateCarbonEmissionData(180) 中的天數參數

# 重新生成數據
mkdir -p data
npx ts-node scripts/generate-seed-data.ts > data/carbon-emissions-seed.json 2>&1
```

## 📝 數據結構

每筆數據記錄包含：

```typescript
{
  date: string;              // ISO 8601 格式的日期
  scope1: number;            // 直接排放（tCO2e）
  scope2: number;            // 能源間接排放（tCO2e）
  scope3: number;            // 其他間接排放（tCO2e）
  totalCarbon: number;       // 總碳排放（tCO2e）
  electricity: number;       // 電力使用（kWh）
  naturalGas: number;        // 天然氣使用（m³）
  fuel: number;              // 燃料使用（L）
  transport: number;         // 運輸距離（km）
  waste: number;             // 廢棄物量（kg）
  water: number;             // 用水量（m³）
  dataSource: string;        // 數據來源
  verified: boolean;         // 是否已驗證
  notes: string;             // 備註
}
```

## ⚠️ 注意事項

1. 這些數據是自動生成的模擬數據，僅供測試和開發使用
2. 實際生產環境應使用真實的碳排放數據
3. 導入數據前會清除該公司的現有碳排放數據（見 `import-seed-data.ts`）
4. 請確保在測試環境中使用，避免覆蓋真實數據

## 🎯 相關文件

- `/prisma/seed.ts`: Prisma 種子腳本
- `/scripts/generate-seed-data.ts`: JSON 數據生成腳本
- `/scripts/import-seed-data.ts`: 數據庫導入腳本
- `/src/app/api/carbon/dashboard/route.ts`: 儀表板 API（支持日期篩選）
- `/src/app/dashboard/page.tsx`: 儀表板前端頁面
