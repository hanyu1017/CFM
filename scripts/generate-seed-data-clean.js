// scripts/generate-seed-data-clean.js
// 生成純 JSON 數據（無額外輸出）
...
function main() {
  ...
  // 寫入文件
  const outputPath = path.join(process.cwd(), 'data', 'carbon-emissions-seed.json');
  ...
  // 輸出摘要（到 stdout，這樣用戶可以看到）
  console.log('✅ 碳排放數據生成完成！');
  console.log(`📄 文件位置: ${outputPath}`);
  console.log(`📊 數據點數量: ${carbonData.length}`);
  console.log(`📅 時間範圍: ${carbonData[0].date.split('T')[0]} ~ ${carbonData[carbonData.length - 1].date.split('T')[0]}`);
  console.log(`📈 總碳排放: ${totalEmissions.toFixed(2)} tCO2e`);
  console.log(`📊 平均每日: ${avgDaily.toFixed(2)} tCO2e`);
  console.log(`📈 最高單日: ${maxDaily.toFixed(2)} tCO2e`);
  console.log(`📉 最低單日: ${minDaily.toFixed(2)} tCO2e`);
  console.log();
