...
function QuickActionCard({ title, description, icon, color, onClick, disabled = false }: QuickActionCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`${colorClasses[color]} text-white rounded-lg p-4 sm:p-6 lg:p-8 min-h-[44px] transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center`}
    >
...
function TabButton({ active, onClick, label }: any) {
  return (
    <button
      onClick={onClick}
      className={`px-3 sm:px-6 py-3 sm:py-4 min-h-[44px] font-medium text-sm sm:text-base transition-colors ${
        active
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="border border-gray-200 rounded-lg p-4 sm:p-6 lg:p-8">
        <h3 className="text-base sm:text-lg font-semibold mb-4">基本資訊</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 sm:px-4 py-2 sm:py-3 min-h-[44px] text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
          <div>
            <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">
      ...
      {/* 內容選擇 */}
      <div className="border border-gray-200 rounded-lg p-4 sm:p-6 lg:p-8">
        <h3 className="text-base sm:text-lg font-semibold mb-4">報告內容選擇</h3>
        <div className="space-y-3 sm:space-y-4">
          {contentSections.map((section) => (
            <label key={section.key} className="flex items-start space-x-3 cursor-pointer min-h-[44px] py-2">
              <input
                type="checkbox"
                checked={config[section.key]}
                onChange={(e) => onConfigChange(section.key, e.target.checked)}
                className="mt-1 w-5 h-5 min-w-[20px] text-blue-600 border-gray-300 rounded focus:ring-blue-500"
              />
              <div className="flex-1">
                <p className="font-medium text-sm sm:text-base text-gray-900">{section.label}</p>
      ...
      <div className="flex gap-3 sm:gap-4">
        <button
          onClick={onGenerate}
          disabled={generating}
          className="flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 min-h-[44px] text-sm sm:text-base bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:bg-gray-400"
        >
...
function ReportHistoryPanel({
  ...
  if (reports.length === 0) {
    return (
      <div className="text-center py-8 sm:py-12 px-4">
        ...
        <p className="text-sm sm:text-base text-gray-600">尚無已生成的報告</p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="space-y-3 sm:space-y-4">
        {reports.map((report) => (
        <div key={report.id} className="border border-gray-200 rounded-lg p-4 sm:p-6 lg:p-8 hover:shadow-md transition-shadow relative">
          {/* 刪除按鈕 - 右上角 */}
          <button
            onClick={() => handleDelete(report)}
            disabled={deleting === report.id}
            className="absolute top-3 sm:top-4 right-3 sm:right-4 p-2 min-h-[44px] min-w-[44px] flex items-center justify-center text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            title="刪除報告"
          >
            <Trash2 className="w-4 sm:w-5 h-4 sm:h-5" />
          </button>

          <div className="flex flex-col sm:flex-row items-start justify-between sm:pr-12 gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 mb-2">
                <h3 className="text-base sm:text-lg font-semibold text-gray-900 break-words">{report.title}</h3>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 mb-4">
                ...
                <span className="break-words">生成於: {new Date(report.createdAt).toLocaleString('zh-TW')}</span>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
                {report.pdfUrl ? (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-4 py-2 sm:py-3 min-h-[44px] text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex-1 sm:flex-none"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-4 py-2 sm:py-3 min-h-[44px] text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex-1 sm:flex-none"
                  >
